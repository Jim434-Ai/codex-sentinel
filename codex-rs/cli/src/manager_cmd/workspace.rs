...
const DEFAULT_AGENT_REGISTRY: &str = "agents.tsv";
...
impl ManagerWorkspace {
    pub(crate) fn load(explicit_root: Option<&Path>) -> anyhow::Result<Self> {
        ...
        let agents = parse_agents_tsv(&registry, &registry_path, &root)?;
...
fn discover_manager_workspace(start_dir: &Path) -> anyhow::Result<PathBuf> {
    for candidate in start_dir.ancestors() {
        if candidate.join(DEFAULT_AGENT_REGISTRY).is_file()
            && candidate.join(MANAGER_AGENTS_FILE).is_file()
        {
    ...
    bail!(
        "could not find manager workspace from {}; expected {} and {}",
        start_dir.display(),
